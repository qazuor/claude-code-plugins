...
count_new_perms() {
    ...
    jq -r ".permissions.${perm_type} // [] | .[]" "$settings_file" 2>/dev/null | while read -r perm; do
...
main() {
    ...
    if [[ "$SYNC_ALL" == "true" ]]; then
        ...
        find "$PROJECTS_ROOT" -maxdepth 4 -type d -name ".claude" ! -path "*/node_modules/*" 2>/dev/null | while read -r claude_dir; do
